...
`axiom_audit_output.txt` records `#print axioms` for the twenty-five
declarations named in Appendix A of the paper. Every one depends only on
`propext`, `Classical.choice` and `Quot.sound`; `TransitParity.H4.transit_parity`
is additionally choice-free. No `sorryAx` and no author-introduced axiom occurs.
